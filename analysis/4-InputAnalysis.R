...
min <- min(childuttdata$numwords)
max <- max(childuttdata$numwords)
mean <- mean(childuttdata$numwords)
median <- median(childuttdata$numwords)
...
plot1.data <- aggregate(childuttdata$numwords, by = c(list(age=childuttdata$age)),FUN=sum)
colnames(plot1.data)[2] <- "total_uttlength"
plot1.data.temp <- aggregate(childuttdata$numwords, by = c(list(age=childuttdata$age)), FUN = function(x){NROW(x)})
