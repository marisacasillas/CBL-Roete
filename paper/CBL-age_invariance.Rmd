...
**Our aim in the present study was to investigate possibility of developmental change in SL by focusing on a single stastical learning mechanism that is proposed be at work in speech segmentation, speech production, and speech comprehension over the longer arc of early language development and, indeed, even into adulthood: backward transitional probability [@mccauley2011learning; @onnis2013language; @pelucchi2009learning; @perruchet2008role]. <!--we are putting an emphasis on continuity here, as that is a good conceptual test of the idea of change with age--> To test for developmental change, we applied a computational learner model that uses BTP to a longitudinal collection of child-caregiver interaction transcripts. This computational modeling approach enabled us to define and test the goodness-of-fit of the BTP-based model across the whole period of interest for early speech production (1;0--4;0), and to therefore check whether BTP's performance changed for each child within the studied developmental range. In what follows, we first explain how and why we chose a model for this preliminary investigation of developmental change in SL. We then explain the evaluation metrics we use for model performance. Finally we describe the model's performance across the tested age range and discuss the implications and limitations of the findings.**

## **Backward transitional probability** and the Chunk-Based Learner

Our model is based on McCauley and Christiansen's [-@mccauley2011learning; -@mccauley2014acquiring; -@mccauley2019language] Chunk-Based Learner (CBL) model, which uses one measure---backward transitional probability [BTP; @perruchet2008role]---to detect statistical dependencies in the speech stream. **BTP for a given pair of words is defined as the occurrence probability of the previous word (w$_{-1}$) given the current word (w$_0$). It can be estimated for each word in a sentence in order to reveal peaks and dips in transitional likelihood, which reflect places where words are likely (peaks) or unlikely (dips) to co-occur. We chose to focus on this particular model for multiple reasons, as outlined below.**
...
-->
**First, we were interested in pursuing a model based on backward transitional probability. BTP is one of multiple approaches for dividing streams of continuous speech into meaningful units; other approaches include, for example, forward transitional probability (FTP) and memory-based chunking [@mareschal2017tracx2; @onnis2013language; @french2011tracx; @pelucchi2009learning; @perruchet2008role; @aslin1998computation; @perruchet1998parser; @saffran1996statistical; @cleeremans1993mechanisms<!--; see @frostInPressinsights for a review-->]. While both BTP and FTP have been shown to effectively enable infants, adults, and simulated learners to segment meaningful chunks from continuous speech, direct comparisons between the two have shown that BTPs outperform FTPs in predicting phonetic word durations in spoken English for both function and content words [@bell2009predictability], in shallow parsing of English, French, and German child-directed speech [@mccauley2019language], and in reconstructing child-produced sentences in 29 languages [@mccauley2019language].**

**Second, among models using BTP, the CBL was of interest in the current study because, at the computational level [@marr1982vision], it is designed to be psycholinguistically plausible (see @mccauley2019language for a review). It uses BTP to incrementally build up an inventory of speech chunks (e.g., "doggy", "look at"), and stores the chunks and their co-occurrences such that the accumulated chunk inventory can be used to both parse and produce utterances on the basis of what it has encountered so far. By only storing shallow information about how chunks combine, its performance in processing multi-chunk utterances also depends exclusively on local relations in the speech signal, mirroring the transitory and sequential nature by which children and adults encounter language [@christiansen2016now]. The central use of chunks and focus on shallow processing in the CBL is consistent with natural and elicited production and comprehension patterns by adults and children [e.g., @arnon2010more; @arnon2011brush; @diessel2000development; @ferreira2007good]. While the model's plausibility only extends to the computational level of analysis—and translations to the algorithmic level will be essential to its long-term utility in understanding language learning [@griffiths2015rational]—its attention to the incremental, local, and structurally parallel nature of natural language use attracted our attention in the present study.**

**Third, the CBL has previously demonstrated success in modeling naturalistic speech production** in multiple corpora, including child language datasets. For example: (a) it parsed text better than a shallow parser in three different languages (English, German and French) when using individual words rather than word classes, (b) it was able to recreate up to 60% of child utterance productions in 13 different languages, and (c) it closely replicated data from an artificial grammar learning study [@mccauley2011learning; @saffran2002constraints]. The model has also been able to replicate experimental data on children's multi-word utterance repetitions [@bannard2008stored], over-regularization of irregular plural nouns [@arnon2011brush], and L2-learner speech [@mccauley2017computational; see also @mccauley2014reappraising **and @mccauley2019language**].

<!--should be addressed for a tighter paper: what would lack of/presence of change in performance imply? how can i link this to the R3 comments below, e.g., about memory changes?-->

**Our broader goal in the current paper was to... In the CBL's use of BTP, orientation toward computational-level psycholinguistic probability, and performance on other speech production tasks, it suited our present needs perfectly. We note, however, that similar longitudinal work with models utilizing other SL mechanisms would be of interest in further work.**

<!--
In sum, the CBL model appears to robustly predict the word-chunk patterns in children's speech when given information about what they hear in their input. We extend this work by testing how the model performs with longitudinal data; it is not yet known how well it functions as a predictor of what children can say as they become more linguistically sophisticated.

For all these reasons—the relative benefits of BTP over FTP, the design of the CBL toward computational-level plausibility, and its performance on related tasks in the past—we felt justified in choosing the CBL to investigate our question about developmental change

BY DOING ALL THESE THINGS THE MODEL HELPS US ACHIEVE SOMETHING COOL/GENERALIZABLE. -- should we focus a bit on the natural link here between segmentation and utterance-level parsing/production?
-->
