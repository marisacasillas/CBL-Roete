...
reconstruct children's speech with age. Our findings suggest that BTP
may be an age-invariant learning mechanism.
...
utterance start cue and then follows that start cue \textbf{with the
chunk that has the highest transitional probability following it, then
adds the next remaining chunk that has the highest transitional
probability following the previous chunk, and again and again, until the
set of chunks for that utterance is exhausted.} So, the set of chunks
\enquote{look at} and \enquote{the puppy} would be ordered depending on
which chunk had the highest BTP with respect to the utterance start cue
(\enquote{look at}), followed by the chunk with the highest BTP with
respect to \enquote{look at} (\enquote{the puppy}).
...
are re-coded as -1.5--1.5) such that the default model output would
reflect the estimated difference from chance at the middle point of our
age range.}
...
of chunks with no duplicates. \textbf{Note that here we are detecting
...
chunks found in the utterance (\protect\hyperlink{fig3}{Figure 3}).
...
the average age in our sample (2;6).\footnote{\textbf{accuracy
  \textasciitilde{} centered.age + (centered.age\textbar{}child).}}
